...
#' y_vars = c("chd","t2d")
#' get_assoc(x=x_vars, y=y_vars, z="+age+sex", d=ukb, model="logistic", get_fit=TRUE)
#'
#' @export
#'
...
#' @param get_fit Logical. Default is FALSE. Get model fit? (lm=R2, logistic=McFadden's pseudo-R2, coxph=Harrell's c-statistic)
#' @param ... Other `tidy_ci()` options
#'
#' @noRd
get_assoc1 = function(x, y, z, d, 
	...
	if (get_fit)  {
		...
		if (logistic)  {
			...
			fit_null = glm(paste0(yy, " ~ 1"), data=d |> select(yy, x, z_vars) |> na.omit(), family=binomial(link="logit"))
