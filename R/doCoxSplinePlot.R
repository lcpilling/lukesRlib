#' Smoothed spline plotting from a CoxPH model
#'
#' @description Plots a smoothed spline curve for the hazard ratio (y) for each value of the primary exposure (x) from a Cox proportional hazard model.
#'
...
#' v0.190611
#'
#' @return A plot of the spline term in a CoxPH model
#'
#' @author Luke Pilling
#'
#' @name doCoxSplinePlot
#'
#' @param x {required} vector of x-values (the primary exposure in the Cox model -- used with pspline() function)
#' @param fit {required} CoxPH object -- fitted model
#' @param x.lab label for x-axis, e.g. "Serum albumin (g/dL)" - whatever your exposure is
#' @param title title for the plot
#' @param subtitle additional subtitle
#' @param returnValues return a data.frame of the x and y values (default is FALSE)
#' @param legendPos default is NO results legend. Specify "topleft" or similar to show the results on the plot
#' @param distBoxPlot show bowplot of distribution? Default is FALSE
#' @param yMax manually set the y-axis maximum? (minimum always 0)
#' @param xLims manually set the x-axis limits? (needs to be a vector of length 2 - min or max can be NULL)
#' @param bg_box_col specific background colour of plot (default = grey98)
#' @param null_line_type horizontal reference line type (default = 2)
#' @param null_line_col horizontal reference line colour (default = grey60)
#' @param null_line_wd horizontal reference line width (default = 1.5)
#' @param h_lines_pos vector of y axis positions for grid lines if desired (default = NULL)
#' @param h_lines_type type of lines for y axis (default = 2)
#' @param h_lines_col colour of lines for y axis (default = grey60)
#' @param h_lines_wd width of lines for y axis (default = 1)
#' @param v_lines_pos vector of x axis positions for grid lines if desired (default = NULL)
#' @param v_lines_type type of lines for x axis (default = 2)
#' @param v_lines_col colour of lines for x axis (default = grey60)
#' @param v_lines_wd width of lines for x axis (default = 1)
#' @param ... other plot options (e.g. cex)
#'
#' @examples
